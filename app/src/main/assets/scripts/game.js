...
Game.prototype.create = function() {
    ...
    draw_button(950 + g, 35, "policy", this);
        ...
        f.button && ("full" === f.name && (this.scale.isFullscreen ? this.scale.stopFullscreen() : this.scale.startFullscreen()), this.tweens.add({
            ...
            onComplete: function() {
                if ("play" === q)
                    ...
                    a(), b();
                else if ("policy" === f.name) {
                      play_sound("click2", d);
                        console.log("Redirecting to GamePolicy.html");
                        openPolicyDialog();
...
function openPolicyDialog() {
    ...
    var dialog = document.getElementById('policyDialog');

    // Make AJAX request to load content from GamePolicy.html
    const xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                // Set the content of the dialog with the response
                dialog.innerHTML = xhr.responseText;

                // Make the dialog scrollable
                dialog.style.maxHeight = '80vh'; // Adjust as needed
                dialog.style.overflowY = 'auto';
                dialog.style.onScroll = 'true'
    ...
    xhr.open('GET', 'GamePolicy.html', true);
    xhr.send();
